...
import kr.co.nogibackend.util.StringUtils;
...
public class HttpLoggingInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
		...
		if (request instanceof CachedBodyHttpServletRequest cachedRequest) {
			...
			log.warn("⚠️ Request is not wrapped with CachedBodyHttpServletRequest. Ensure CacheBodyFilter is applied.");
		}
		return true;
	}
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler,
		Exception ex) {
		long startTime = requestTimestamps.getOrDefault(getRequestKey(request), System.currentTimeMillis());
