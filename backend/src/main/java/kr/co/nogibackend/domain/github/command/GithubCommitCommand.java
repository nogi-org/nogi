...
public record GithubCommitCommand(
		...
		SyncHistoryType type,       // 히스토리 타입 (생성, 수정 등)
	...
	public String getCommitMessage() {
		...
		return """
				...
				type == SyncHistoryType.CREATE_OR_UPDATE_CONTENT ? "작성" : "수정");
	}
	...
	private void addHistoryFile(Map<String, String> fileMap) {
		if (
			// 제목 or 카테고리가 수정되었을 경우 이전 md file 을 삭제하기 위해 null 값을 value 로 설정
				type == SyncHistoryType.UPDATE_TITLE ||
						type == SyncHistoryType.UPDATE_CATEGORY
		) {
