...
import kr.co.nogibackend.application.user.command.UserFacadeCommand.NotionLogin;
...
public class NotionAuthControllerV1 {
	...
	public ResponseEntity<?> loginByNotion(
		...
		UserLoginByNotionResult userLoginByNotionResult = userLoginFacade.loginByNotion(
				new NotionLogin(userId, notionClientId, notionClientSecret, notionRedirectUri, code)
		);

		String redirectUrl = new StringBuilder(afterLoginRedirectUrl)
				.append("?isSuccess=").append(userLoginByNotionResult.isSuccess())
				.append("&message=").append(encodeParam(userLoginByNotionResult.message()))
